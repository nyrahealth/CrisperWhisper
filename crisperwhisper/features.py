"""Log-mel feature extraction for Whisper models.

Vendored unmodified from faster-whisper 1.2.1 (``faster_whisper/feature_extractor.py``),
MIT License, Copyright (c) 2023 SYSTRAN, https://github.com/SYSTRAN/faster-whisper.
Vendoring removes the ``faster-whisper`` dependency, whose transitive requirement on
upstream ``ctranslate2`` clobbers the ``ctranslate2-crisperwhisper`` fork in
site-packages (both distributions own the ``ctranslate2`` import package).

The implementation is pure numpy and matches Whisper's expected frame count exactly.
Numerical equivalence with the upstream class is enforced by
``tests/test_features.py`` (bit-identical output on real audio).
"""
