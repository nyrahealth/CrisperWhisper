...
class WhisperXModel(SpeechRecognitionModel):
    ...
    def _format_output(self, transcript: Dict[str, Any]) -> ModelOutput:
        ...
        timestamped_outputs = [
            TimestampedOutput(
                word=entry["word"], start=entry["start"], end=entry["end"]
            )
            for entry in transcript["word_segments"]
        ]

        prediction_str = " ".join(
            [entry["word"] for entry in transcript["word_segments"]]
        )
        return ModelOutput(
    ...
    def __call__(
        self, audio_path: Union[str, Path, List[float]], batch_size: int = 1
    ) -> ModelOutput:
...
class WhisperTimestamped(SpeechRecognitionModel):
    ...
    def __init__(self, size: str, language_code: str = "en", device: str = "cuda"):
...
def get_model(
    model_config: dict[str, Any], device: str = "cuda"
...
def transcribe_single_file(
    model: SpeechRecognitionModel, audio_path: str
) -> ModelOutput:
...
def transcribe_speech_files(
    model: SpeechRecognitionModel, dataset_name: str, audio_paths: List[str]
